...
export default function ReviewCandidateForR2FormFooter({task}: { task: FullHumanTask }) {
    ...
    return <TaskSubmitFooter isCommentRequired={true} task={task} buttons={[
        {
            ...
            onClick: (e, comment) => {
                ...
                executeUserTask(task.id, {bodComment: comment, isPassed: true,}, true
                ).then(response => {
        ...
        },
        {
            ...
            onClick: (e, comment) => {
                e.preventDefault()
                executeUserTask(task.id, {bodComment: comment, isPassed: false,}, true).then(response => {
                    if (response.status === 204) {
